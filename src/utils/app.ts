...
export class SocialMediaApp {
  ...
  private setupFormListeners(): void {
    const platforms: SocialMediaPlatform[] = ['youtube', 'linkedin', 'twitter', 'instagram', 'tiktok'];
  ...
  private async handleFormSubmission(platform: SocialMediaPlatform, form: HTMLFormElement): Promise<void> {
    const transcript = this.sharedTranscript.value;
    const transcriptError = validateTranscript(transcript);
    ...
    const hiddenTranscript = form.querySelector(`#${this.getPlatformPrefix(platform)}-transcript`) as HTMLTextAreaElement;
    if (hiddenTranscript) {
